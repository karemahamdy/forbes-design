<template>
  <div>
    ...
    <div v-if="activeCategory || searchQuery" class="animate-fade-up mb-6 flex items-center justify-between border-b border-[#111111] pb-4 transition-colors duration-300 dark:border-white">
      <h2 class="font-sans text-[13px] font-semibold uppercase tracking-widest text-[#111111] transition-colors duration-300 dark:text-white">
    ...
    <div v-else-if="error" class="animate-fade-up py-16 text-center">
      ...
      <button @click="refresh()" class="theme-toggle bg-[#111111] px-4 py-2 text-xs font-semibold text-white hover:bg-[#c00000] dark:bg-white dark:text-[#050505] dark:hover:bg-[#ff3333] dark:hover:text-white">Retry</button>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="featured && !activeCategory && !searchQuery" class="motion-card animate-fade-up mb-8 border-t-4 border-[#c00000] bg-white transition-colors duration-300 dark:border-[#ff3333] dark:bg-[#050505]">
        <div
          class="cursor-pointer group"
          @click="navigateTo(`/article/${featured.slug}`)"
        >
          <!-- Hero image -->
          ...
          <div class="p-6 md:p-8">
            <!-- Author row -->
            <div class="mb-4 flex items-center gap-3 border-b border-t border-[#111111] py-4 transition-colors duration-300 dark:border-white">
              <img
              ...
              />
              <div>
                <div class="font-sans text-[20px] font-bold uppercase tracking-wider text-[#111111] transition-colors duration-300 dark:text-white">{{ featured.author.name }}</div>
                <div class="flex items-center gap-1 font-sans text-[20px] text-black transition-colors duration-300 dark:text-white/70">
            ...
            <!-- Title -->
            <h1 class="animated-rule mb-4 border-b border-[#111111] pb-2 font-sans text-[22px] font-bold uppercase leading-tight text-[#111111] transition-colors group-hover:text-[#c00000] dark:border-white dark:text-white dark:group-hover:text-[#ff3333] sm:text-[28px]">
              {{ featured.title }}
            </h1>
